...
class WithdrawMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if Bankrupt.objects.filter(isBankrupt=True).exists():
            messages.error(
                self.request,
                'Unable to withdraw money. The bank is currently bankrupt.'
            )
...
class TranferMoney(TransactionCreateMixin):
    ...
    #         return render(request, self.template_name,{'title':'Transfer Money'})
    def form_valid(self, form):
        amount = form.cleaned_data.get('amount')
        transfer_account_no = form.cleaned_data.get('Transfer_account_no')
        try:
            transfer_account = UserBankAccount.objects.get(account_no=transfer_account_no)
        except UserBankAccount.DoesNotExist:
            messages.error(
                self.request,
                'Transfer account not found. Please check the account number.'
            )
            return redirect('transaction_report')
...
# class TranferMoney(LoginRequiredMixin,View):
#   pass